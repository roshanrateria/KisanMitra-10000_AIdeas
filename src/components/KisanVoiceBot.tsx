...
export function KisanVoiceBot() {
  ...
  // TTS via Web Speech API + simulated lip-sync animation
  const speak = useCallback((text: string) => {
    if (!window.speechSynthesis) { setState('idle'); return; }
    window.speechSynthesis.cancel();
    const utt = new SpeechSynthesisUtterance(text);
    utt.lang = language === 'hi' ? 'hi-IN' : language === 'pa' ? 'pa-IN' : language === 'gu' ? 'gu-IN' : language === 'ta' ? 'ta-IN' : language === 'te' ? 'te-IN' : language === 'mr' ? 'mr-IN' : language === 'bn' ? 'bn-IN' : 'en-IN';
    ...
    utt.onend = () => { stopLipSync(); setState('idle'); };
    utt.onerror = () => { stopLipSync(); setState('idle'); };
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      const botMsg: Message = { id: (Date.now() + 1).toString(), role: 'bot', text: reply, ts: Date.now() };
  ...
  useEffect(() => () => { cleanup(); window.speechSynthesis?.cancel(); }, [cleanup]);

  return (
    <div className="grid md:grid-cols-5 gap-6 h-[calc(100vh-240px)] min-h-[600px]">
      {/* Left: Avatar panel */}      <div className="md:col-span-2 flex flex-col items-center justify-center bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 rounded-2xl border shadow-sm p-6 gap-4">
      ...
      {/* Right: Chat panel */}
      <div className="md:col-span-3 flex flex-col rounded-2xl border shadow-sm overflow-hidden bg-white dark:bg-slate-900">
        {/* Chat header */}
