...
export function VoiceChatbot() {
    ...
    return (
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className="group relative flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 shadow-2xl hover:scale-110 active:scale-95 transition-transform duration-200"
  ...
  // ── Expanded chat panel ──────────────────────────────────────────
  return (
    <div className="fixed bottom-6 right-6 z-50 w-[380px] max-w-[calc(100vw-1.5rem)] flex flex-col shadow-2xl rounded-2xl overflow-hidden border border-border/60 bg-background">
      {/* Header */}
        ...
        <div className="space-y-3">
          {msgs.map(m => (
            <div key={m.id} className={cn('flex gap-2', m.role === 'user' ? 'flex-row-reverse' : 'flex-row')}>
              ...
              <div className={cn('max-w-[78%] px-3 py-2 rounded-2xl text-sm',
                m.role === 'user'
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-background border-t border-border/50">
        ...
        <div className="relative">
          ...
          <button
            ...
            disabled={isProcessing}
            className={cn(
              'relative z-10 flex items-center justify-center w-14 h-14 rounded-full shadow-lg transition-all duration-200 active:scale-95 disabled:opacity-50',
              isRecording
